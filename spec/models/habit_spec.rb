...
RSpec.describe Habit, type: :model do
  describe 'バリデーション' do
    ...
    it 'titleが255文字以下であること' do
      habit.title = 'a' * 256
      expect(habit).to_not be_valid
      expect(habit.errors[:title]).to include("is too long (maximum is 255 characters)")
    end

    it 'descriptionが1000文字以下であること' do
      habit.description = 'a' * 1001
      expect(habit).to_not be_valid
      expect(habit.errors[:description]).to include("is too long (maximum is 1000 characters)")
    end
